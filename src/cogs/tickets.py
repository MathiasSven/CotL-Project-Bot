...
class Tickets(commands.Cog):
    ...
    async def create_application_embed(self) -> discord.Message:
        ...
        message = await self.bot.APPLICATION_CHANNEL.send(embed=embed)
    ...
    async def create_interviewer_embed(self, applicant: discord.Member) -> discord.Message:
        embed = discord.Embed(title="New Applicant", colour=discord.Colour(self.bot.COLOUR),
                              description=f"User: {applicant.mention}\n"
                                          f"The first on the trigger, react with {EMOJI[':white_check_mark:']}")
        embed.set_footer(text="Unassigned")
    ...
    async def startup(self):
        ...
        messages_w_embeds = [message async for message in self.bot.APPLICATION_CHANNEL.history(limit=100) if bool(message.embeds)]
        try:
        ...
        while True:
            ...
            message = await self.bot.APPLICATION_CHANNEL.fetch_message(payload.message_id)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        try:
            ...
            return
        if "New Applicant" not in interviewer_embed.title:
            return

        await message.clear_reactions()
        interviewer_embed.timestamp = datetime.datetime.utcnow()
        interviewer_embed.set_footer(text=f"Assigned to {member.display_name}")
